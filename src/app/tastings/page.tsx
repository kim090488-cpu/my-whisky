...
export const metadata = {
  title: "테이스팅 노트 · my-whisky",
  description: "공개된 위스키 테이스팅 노트를 사진과 함께 둘러보세요.",
};
...
export default async function TastingsPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const userIds = Array.from(new Set(tastingsRaw.map((t) => String(t.user_id))));
  ...
  const hasFilters =
    country !== null || minScore !== null || buyback || flavors.length > 0;

  return (
    <main className="mx-auto max-w-6xl px-4 py-6 sm:py-10">
      <header className="mb-6 flex items-end justify-between gap-3">
        <div>
          <h1 className="font-serif text-3xl tracking-tight sm:text-4xl">테이스팅 노트</h1>
          <p className="mt-1 text-xs text-muted-foreground">
            공개된 후기 · {total.toLocaleString()}개{hasFilters && " (필터됨)"}
          </p>
        </div>
        {user && (
          <Link
